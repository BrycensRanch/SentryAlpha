...
let guildSettingsChannel = client.configChannels.get(member.guild.id)
    ...
    await client.channels.fetch(guildSettingsChannel).catch(() => null).then(channel => {
        ...
        .setDescription(`**${member.user.tag}** (${member.id}) has lost the role:\n  **${role.name}**!\nOld Roles: ${member.roles.cache.sort((a, b) => b.rawPosition - a.rawPosition).filter(roles => roles.position !== 0).map(role => `${role}`)}\n\nNew Role: ${role.toString()}`)
        .addField("Removed By:" , `${member.guild.me.permissions.serialize().VIEW_AUDIT_LOG ? `${exeUser.id !== member.id ? `${exeUser.tag} (${exeUser.id})` : "Themselves."}` : "Unknown, please give me the \`View Audit Log\` permission!"}`)
